...
export const mutations = {
  [types.FETCH_USER_CREDIT] (state, { credits, isLoading }) {
    state.credits.data = credits
    state.credits.isLoading = isLoading
  },
...
export const actions = {
  ...
  async fetchUserCredits ({ commit }) {
    try {
      commit(types.FETCH_USER_CREDIT, { credits: [], isLoading: true })

      const { data } = await axios.get('/api/credit/list')
      commit(types.FETCH_USER_CREDIT, { credits: data.data, isLoading: false })
    } catch (e) {
      return e
    }
  },
